...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
# Modelo Arima sin transformaciones
predicciones1= auto.arima(Data_ts$y)

# Forecast Arima sin transformaciones
predicciones1_forec= forecast(predicciones1, h=12)
...
hw_accuracy <- compute_accuracy(actual_values, tail(Modelo_evaluation$fitted, n = 12))
arima_accuracy <- compute_accuracy(actual_values, tail(predicciones1$fitted, n = 12))
...
last_value_original <- tail(x$homicidios, 1)

predicciones2$mean <- cumsum(c(last_value_original, predicciones2$mean))[-11]

predicciones2$mean=exp(predicciones2$mean)
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
fecha_Inicial= as.Date("2024-02-01")

Fechas_Predicciones= seq(fecha_Inicial, by= "month", length.out= length(Modelo_evaluation$mean ))

hw_df= data.frame(Periodo= seq(fecha_Inicial, by= "month",length.out= length(Modelo_evaluation$mean)), Prediccion= Modelo_evaluation$mean, Lower=Modelo_evaluation$lower, Upper= Modelo_evaluation$upper)

Arima_df= data.frame(Periodo= seq(fecha_Inicial, by= "month",length.out= length(predicciones2$mean)), Prediccion= predicciones2$mean, Lower=predicciones2$lower, Upper= predicciones2$upper)

Prophet_df= data.frame(Periodo= seq(fecha_Inicial, by= "month",length.out= length(tail(forecast_prophet$yhat,11))), Prediccion= tail(forecast_prophet$yhat, 11))

```

## Comparacion Pronosticos 2024
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
combined_forecast= data.frame(Periodo= Fechas_Predicciones)

combined_forecast$HW= hw_df$Prediccion

combined_forecast$Arima= Arima_df$Prediccion

combined_forecast$Prophet= Prophet_df$Prediccion

as.matrix.data.frame( combined_forecast)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
ggplot(data = combined_forecast)+
  geom_line(data = Data_ts, aes(x= ds, y= y), color= "black", size= 0.5)+
  geom_line(aes(x= Periodo, y= HW, color="Holt-Winter"))+
  geom_line(aes(x= Periodo, y= Arima, color="Arima"))+
  geom_line(aes(x= Periodo, y= Prophet, color="Prophet"))
  
  labs(title = "comparacion de Predicciones de Diferentes Modelos",
       x= "Fecha",
       y= "Prediccion",
       color= "Modelo")+
  theme_minimal()
